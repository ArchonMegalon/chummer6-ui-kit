...
- Tightened `ClassicDenseWorkbenchPreset` so proof-family lists must stay exactly canonical and chrome-regression sentinels are validated against the live preset field values, including the combined menu/toolstrip top-chrome ceiling, instead of stale copied strings.
